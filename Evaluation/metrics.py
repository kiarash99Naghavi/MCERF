...
def eval_retrieval_qa(results_csv):
    ...
    for i, row in results_df.iterrows():
        ...
        # if the score is low, store the row index
        if f1_scores[-1] < 0.5:
            low_f1_indices.append(i)
            low_question_indices.append(row['question'])
            low_score.append(f1_scores[-1])
...
def eval_compilation_qa(results_csv):
    ...
    for i, row in results_df.iterrows():
        try:
            prediction_tokens = row['model_prediction'].replace(" ", "").split(",")
...
def eval_functional_performance_qa(results_csv):
    ...
    return mean_score(accuracies), accuracies, mean_score(bleus), bleus, mean(rogues), rogues, mean_score(similarities)
