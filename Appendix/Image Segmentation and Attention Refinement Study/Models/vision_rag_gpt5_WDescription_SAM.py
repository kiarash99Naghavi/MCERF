...
class VisionRAG:
    ...
    def search(self, text_query: str, index_name: str, k: int = 3) -> List[RAGResult]:
        ...
        raw = self._rag_model.search(query=text_query, k=k, return_base64_results=True)
    ...
    def answer_query(
        ...
        external_image_dir: Optional[str] = None,  # ROI directory appended AFTER question
    ) -> str:
        if not self._index_exists(self.index_root, index_name):
            if fallback_input_path:
                ...
                raise FileNotFoundError(
                    ...
                    f"Build it or pass fallback_input_path to auto-build."
                )
        ...
        # Use CSV question if available, else image description, else plain query
        user_question = (csv_question or imgDiscription or query or "").strip()
        if not user_question:
            raise ValueError("No question or description available.")

        # Retrieval may benefit from the explicit question
        ...
        # Order: pages (+original) → text question → ROI images
        messages = [
            {"role": "system", "content": [{"type": "text", "text": system_prompt}]},
            {
                "role": "user",
                "content": image_parts + roi_image_parts +[{"type": "text", "text": user_question}]+[{"type": "text", "text": imgDiscription}],
            },
        ]
        ...
        if not content:
            fallback_msgs = [
                {"role": "system", "content": [{"type": "text", "text": system_prompt}]},
                {"role": "user", "content": [{"type": "text", "text": user_question}] + image_parts+[{"type": "text", "text": imgDiscription}]},
            ]
