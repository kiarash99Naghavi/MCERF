...
"""
VisionRAG: Retrieval-Augmented Generation over PDFs with GPT-4o-MCERF Fine-Tuned
"""
...
#     raise ImportError("vision_rag_gpt5 requires `byaldi`. Install it with `pip install byaldi`.") from exc
...
class VisionRAG:
    ...
    def search(self, text_query: str, index_name: str, k: int = 3) -> List[RAGResult]:
        ...
        # for r in raw:
        #     print({"doc_id": r.doc_id, "score": r.score, "page": r.page_num})
        # with open("results.txt", "w") as f:
        #     # f.write(str([RAGResult(doc_id=r.doc_id, score=r.score, base64=None) for r in raw]))
        #     f.write(str(raw))
    ...
    def answer_query(
        ...
        # If True and CSV is given, prefer CSV prompt over `query`.
        ...
        # 2) Retrieve relevant PDF pages (will only load index if not already loaded)
        ...
        messages = [
            ...
            {"role": "user", "content": image_parts + [{"type": "text", "text": user_question}]},
        ]
        ...
        resp = client.chat.completions.create(
            model="ft:",  # Use the fine-tuned model name here
...
if __name__ == "__main__":
    ...
    #     print({"doc_id": result.doc_id, "score": result.score, "page": result.page_num})
